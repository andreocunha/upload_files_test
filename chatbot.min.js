...
let content=null;let input=null;let inputArea=null;async function sendEmail(){await fetch('https://disparador-emails-moddev.vercel.app/api/email',{method:'POST',headers:{'Accept':'application/json, text/plain, */*','Content-Type':'application/json','Access-Control-Allow-Credentials':'true','Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET, POST, PUT, DELETE, OPTIONS','Access-Control-Allow-Headers':'Origin, X-Requested-With, Content-Type, Accept'},body:JSON.stringify({body:`
      ...
      `,subject:'Cotação para empresa'+answersResult[4],email:answersResult[2],})}).then(response=>response.json()).then(data=>{console.log(data);}).catch(error=>console.log(error));}
...
onclick="removeOptions(this, '${option}')">${option}</button>`).join('')}
  ...
  `;}
else{sendEmail();div.innerHTML=`<p>Nossos consultores estão analisando sua cotação e em breve entraremos em contato. Obrigado! Caso queira entre em contato conosco por nosso Whatsapp.</p>`;content.appendChild(div);content.innerHTML+=`
